...
class AddonsController extends Controller
{
	...
	function update(Request $request)
	{
		...
		$output = null;
		if ($request->status == 1) {
			$output = AddonsModel::where('id', $request->id)->update(['is_active' => 0]);
		} else if ($request->status == 0) {
			$output = AddonsModel::where('id', $request->id)->update(['is_active' => 1]);
		}
	...
	function store(Request $request)
	{
		...
		if (empty($request->id)) {
			$output = AddonsModel::create($data);
			if ($output) {
				$content = [
					'code' => 200,
					'status' => true,
			...
			$output = AddonsModel::where('id', $request->id)->update($data);
			if ($output) {
				$content = [
					'code' => 200,
					'status' => true,
