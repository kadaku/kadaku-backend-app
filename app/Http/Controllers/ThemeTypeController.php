...
use App\Models\ThemeTypeModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class ThemeTypeController extends Controller
{
	function index()
	{
		$data['title'] = 'Themes Type';
		return view('theme_type.index', $data);
	}
	...
	function update(Request $request)
	{
		...
		$output = null;
		if ($request->status == 1) {
			$output = ThemeTypeModel::where('id', $request->id)->update(['is_active' => 0]);
		} else if ($request->status == 0) {
			$output = ThemeTypeModel::where('id', $request->id)->update(['is_active' => 1]);
		}
	...
	function store(Request $request)
	{
		...
		if (empty($request->id)) {
			$output = ThemeTypeModel::create($data);
			if ($output) {
				$content = [
					'code' => 200,
					'status' => true,
			...
			$output = ThemeTypeModel::where('id', $request->id)->update($data);
			if ($output) {
				$content = [
					'code' => 200,
					'status' => true,
	...
	{
		$output = ThemeTypeModel::destroy($id);
		if ($output) {
			$content = [
				'code' => 200,
				'status' => true,
