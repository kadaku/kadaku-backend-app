...
function paginationJump(total_data, limit, page, tab) {
    ...
    for (var p = start; p <= finish; p++) {
        if (p !== page) {
            ...
            pageNumber += '<li class="active"><input min="1" class="form-control-paging" onkeyup="mustNumber(this)" type="number" value="' + page + '" style="width:50px"/><button type="button" style="display:inline-block" class="btn btn-primary btn-sm mb-1" title="Lompat ke halaman" onclick="gotopage(this, ' + tab + ')"><i class="bx bx-search"></i></button></li>' + '';
        }
    }
...
function syamValidationServer(element, name, data) {
    if (data.data_error[name]) syamValidation(element, data.data_error[name])
}

function syamValidationGroupServer(element, name, data) {
    if (data.data_error[name]) syamValidationGroup(element, data.data_error[name])
}

function syamValidationServerSelect(element, name, data) {
    if (data.data_error[name]) syamValidationSelect(element, data.data_error[name])
}
...
function pleaseWait() {
    return `<div style="margin: auto;border: 0;width: 150px;padding: 4px 10px;text-align:center;">
                <img src="storage/images/loaders/spinner-loader.gif" style="margin:10px;auto;" width="70%">
                <p style="font-size:16px;font-weight:bold;text-align:center;">Please Wait...</p>
            </div>`;
}
