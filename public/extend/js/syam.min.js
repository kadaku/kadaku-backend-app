...
function countWords(element_input, element_show) {
	s = $(element_input).val();
	s = s.replace(/(^\s*)|(\s*$)/gi,"");
	s = s.replace(/[ ]{2,}/gi," ");
	s = s.replace(/\n /,"\n");
	if ($(element_input).val() === '') {
			return $(element_show).text(0); 
	} else {
			return $(element_show).text(s.split(' ').length); 
	}
}

function countCharacters(element_input, element_show) {
	s = $(element_input).val();
	if (s === '') {
			return $(element_show).text(0); 
	} else {
			return $(element_show).text(s.length); 
	}
}
